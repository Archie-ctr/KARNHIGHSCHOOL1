...
<div class="portal-content">
  ...
  <div class="page-heading">
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap">
      <a href="<?= BASE_URL ?>/letters/report_card_pdf.php?student_id=<?= $student['id'] ?>&ay_id=<?= $ayId ?>"
  ...
  <div id="rcView" class="report-card-paper">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px 20px;font-size:12px;margin-bottom:14px;padding:10px;background:var(--bg);border-radius:6px">
      <div><strong>Student:</strong> <?= e(trim($student['first_name'].($student['middle_name']?' '.$student['middle_name']:'').' '.$student['last_name'])) ?></div>
    ...
    <table class="rc-table">
      ...
      <tbody>
        <?php foreach ($subjects as $sub):
          $cols = [
              '1st Period'=>'—','2nd Period'=>'—','3rd Period'=>'—','Semester 1 Examination'=>'—',
              '4th Period'=>'—','5th Period'=>'—','6th Period'=>'—','Semester 2 Examination'=>'—',
          ];
          $vals = [];
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin:14px 0;font-size:12px">
      <div style="text-align:center;padding:8px;background:var(--bg);border-radius:6px">
        <strong style="display:block;font-size:16px"><?= $rc['days_present'] ?? 0 ?></strong>Days Present
      </div>
      <div style="text-align:center;padding:8px;background:var(--bg);border-radius:6px">
        <strong style="display:block;font-size:16px"><?= $rc['days_absent'] ?? 0 ?></strong>Days Absent
      </div>
      <div style="text-align:center;padding:8px;background:var(--bg);border-radius:6px">
        <strong style="display:block;font-size:16px"><?= $rc['days_tardy'] ?? 0 ?></strong>Times Tardy
      </div>
      <div style="text-align:center;padding:8px;background:var(--primary-soft);border-radius:6px">
        <strong style="display:block;font-size:16px;color:var(--primary)">
          <?= $rc['yearly_average'] ? round($rc['yearly_average'],1).'%' : '—' ?>
        </strong>Overall Avg
      </div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;font-size:12px;margin-bottom:12px">
      <?php if ($rc['teacher_comment']): ?>
      <div style="padding:8px;border:1px solid var(--line);border-radius:5px">
        <strong>Teacher:</strong> <?= nl2br(e($rc['teacher_comment'])) ?>
      </div>
      <?php endif; ?>
      <?php if ($rc['principal_comment']): ?>
      <div style="padding:8px;border:1px solid var(--line);border-radius:5px">
        <strong>Principal:</strong> <?= nl2br(e($rc['principal_comment'])) ?>
      </div>
      <?php endif; ?>
    </div>
    <?php endif; ?>
    ...
    <div style="font-size:13px;font-weight:700;color:var(--primary);margin-bottom:14px">
      Promotion: <?= e($rc['promotion_status'] ?? 'Pending') ?>
    </div>
    ...
    <div style="text-align:center;margin-top:12px;font-size:10px;color:var(--ink-faint)">
      <?= e($schoolName) ?> &middot; Karnplay, Nimba, Liberia
    </div>
  </div>
  <?php endif; ?>
