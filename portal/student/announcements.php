...
$student = $pdo->prepare("SELECT id, first_name FROM students WHERE user_id=? LIMIT 1");
$student->execute([$user['id']]);
$student = $student->fetch();
if (!$student) { redirect(BASE_URL.'/portal/student/'); }

// Get all active announcements targeting all or students
try {
    $anns = $pdo->query(
...
<div class="portal-content">
  ...
  <div class="page-heading">
    <div>
      <h1>Announcements</h1>
      <p>School notices, events and important information</p>
    </div>
    <?php if (!empty($anns)): ?>
  ...
  <div style="text-align:center;padding:56px;background:var(--surface);border:1px solid var(--line);border-radius:var(--radius)">
    ...
    <p style="color:var(--ink-soft)">There are no announcements at this time. Check back soon.</p>
  </div>
    ...
    <article style="background:#fff;border:1px solid var(--line);border-radius:var(--radius);overflow:hidden">
      ...
      <div style="padding:14px 20px">
        ...
        <div style="display:flex;align-items:center;gap:16px;margin-top:12px;font-size:11.5px;color:var(--ink-faint)">
          ...
          <span>👤 <?= e($ann['posted_by_name']) ?></span>
          <?php endif; ?>
          <?php if (!empty($ann['expires_at'])): ?>
          <span>⏳ Expires <?= date('M d, Y', strtotime($ann['expires_at'])) ?></span>
          <?php endif; ?>
        </div>
      </div>
    </article>
